...
import csv
from sparseldamatrix_topics import *
...
def csvsparsematrix_topics (filepath, data_row, data_col, outfilename, num_topics, num_words_per_topic, question_num=None, pos_list=[]):
    ...
    with open(filepath) as file_obj:
                ...
                if index < data_col:#Skip question number
                    ...
                    if clean_cell_str != "" and clean_cell_str != " ":
                        ...
                        for word in clean_cell_word_list:
                            ...
                            response_words_list[list_index].append(word)
                            # print("appending", word, list_index)
        # print(response_words_list)

        #Call the ldamulticoretopics function
        data_list = []
        if question_num == None:
            # print("in after if")
            data_list = response_words_list
        else:
            # print("in after else")
        ...
        if data_list == []:
            ...
            return False
        sparseldamatrix_topics(None, num_topics, outfilename, [], data_list)
        return True
...
pos_list = ["NOUN", "ADJ"]
outfilename = "csvsparsematrix_topics_all.txt"
num_topics = 8
num_words_per_topic = 3
question_num = None
csvsparsematrix_topics (filepath, data_row, data_col, outfilename, num_topics, num_words_per_topic, question_num, pos_list)

for index in range(8): #Include 7
    question_num = index + 1
    num_topics = 5
    num_words_per_topic = 3
    outfilename = f"csvsparsematrix_topics_question{question_num}.txt"
    csvsparsematrix_topics (filepath, data_row, data_col, outfilename, num_topics, num_words_per_topic, question_num, pos_list)
...
# data_col = 0 #Skip nothing
# pos_list = ["NOUN", "ADJ"]
# outfilename = "csvsparsematrix_topics_question8.txt"
# num_topics = 5
# num_words_per_topic = 3
# question_num = 8
# csvsparsematrix_topics (filepath, data_row, data_col, outfilename, num_topics, num_words_per_topic, question_num, pos_list)
